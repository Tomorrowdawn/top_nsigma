...
class RequestStateTracker:
    """Tracks states for sequences using seq_id as global unique identifier"""

    def __init__(self):
        # Map seq_id -> state
        self._states: Dict[int, Any] = {}
        # Current batch's mapping: position -> seq_id
        self._curr_batch_ids: Optional[torch.LongTensor] = None

    def update(self, sampling_metadata: SamplingMetadata):
        """Update tracker state based on sampling metadata"""
        # Build position -> seq_id mapping for current batch
        sample_ids = []
        for seq_group in sampling_metadata.seq_groups:
            for seq_id, _ in zip(seq_group.seq_ids, seq_group.sample_indices):
                sample_ids.append(seq_id)

        self._curr_batch_ids = torch.tensor(sample_ids, dtype=torch.long, device="cuda")

    def get_seq_uids(self) -> torch.Tensor:
        """Get sequence IDs tensor for current batch positions"""
        assert self._curr_batch_ids is not None, "Must call update() first"
        return self._curr_batch_ids


@dataclass
class MirostatState:
    """State for tracking mirostat parameters per sequence."""

    max_surprise: float  # Maximum allowed surprise value
    running_tot_surprise: float  # Running total of surprise values
    error_surprise: float  # Error between target and actual surprise
    learning_rate: float = 1.0  # Learning rate for adjusting max_surprise


class FacadeSampler(nn.Module):
    def __init__(
        self,
        device: str = "cuda",
        top_nsigma: float = 0.0,  # top-nsigma sampling
        eta_eps: float = 0.0,  # eta sampling
        mirostat_tau: float = 0.0,  # mirostat sampling
    ):
        super().__init__()
        ...
        assert xor(
            eta_eps > 0, mirostat_tau > 0, top_nsigma > 0
        ), "Only one of eta_eps, mirostat_tau, top_nsigma can be greater than 0"
        # Whether or not the SamplerOutput should have on-device tensors
        # containing the sampled token ids and probabilities. This is used by
        # speculative decoding.
        self.include_gpu_probs_tensor = False
        self.should_modify_greedy_probs_inplace = False
        self.nsigma = torch.tensor(top_nsigma, device=device)
        self.eta_eps = torch.tensor(eta_eps, device=device)
        self.mirostat_tau = torch.tensor(mirostat_tau, device=device)
        self.sampling_method = "top-nsigma" if top_nsigma > 0 else "mirostat" if mirostat_tau > 0 else "eta"
        self.tracker = None
        if self.sampling_method == "mirostat":
            self.tracker = RequestStateTracker()
        print(f"nsigma: {self.nsigma}")

    def nsigma_truncate(self, logits: torch.Tensor):
        std = logits.std(dim=-1, keepdim=True)
        threshold = logits.max(dim=-1, keepdim=True).values - self.nsigma * std
        logits = torch.where(logits > threshold, logits, float("-inf"))
        return logits

    def eta_truncate(self, logits: torch.Tensor):
        # Compute entropy of logits distribution
        probs = torch.softmax(logits, dim=-1)
        entropy = -torch.sum(probs * torch.log(probs + 1e-10), dim=-1, keepdim=True)
        # Compute threshold as min(eps, alpha * exp(-entropy))
        # Where alpha = sqrt(eps) as suggested in the paper
        alpha = torch.sqrt(self.eta_eps)
        threshold = torch.minimum(self.eta_eps, alpha * torch.exp(-entropy))
        # Filter logits below threshold
        logits = torch.where(probs > threshold, logits, float("-inf"))
        return logits

    def mirostat_init(self, sampling_metadata: SamplingMetadata):
        """Initialize mirostat state for new sequences."""
        # Update tracker with current batch info
        assert self.tracker is not None
        self.tracker.update(sampling_metadata)
        # Initialize new sequence states
        for seq_group in sampling_metadata.seq_groups:
            if seq_group.do_sample:
                for seq_id in seq_group.seq_ids:
                    if seq_id not in self.tracker._states:
                        # Initialize with 2 * target surprise as max_surprise
                        self.tracker._states[seq_id] = MirostatState(
                            max_surprise=2.0 * self.mirostat_tau.item(),
                            running_tot_surprise=0.0,
                            error_surprise=0.0,
                        )

    def mirostat_truncate(self, logits: torch.Tensor) -> torch.Tensor:
        """Compute truncation point using mirostat algorithm."""
        # Get sequence IDs for current batch positions
        assert self.tracker is not None
        seq_ids = self.tracker.get_seq_uids()
        ...
        for i, seq_id in enumerate(seq_ids):
            if seq_id.item() in self.tracker._states:
                ...
                # Estimate parameter s (slope of log-prob distribution)
                s = self._estimate_s(sorted_probs)

                # Compute k (number of tokens to keep)
                n = probs.size(-1)  # vocab size
                k = self._compute_k(n, s, state.max_surprise)
                ...
                logits[i] = torch.where(mask, logits[i], torch.tensor(float("-inf"), device=logits.device))

        return logits

    def _estimate_s(self, probs: torch.Tensor) -> float:
        """Estimate the parameter s from probability distribution."""
        num = 0.0
        den = 0.0
        for i in range(min(100, len(probs) - 1)):
            b = (probs[i] / probs[i + 1]).item()
            t = (i + 2) / (i + 1)
    ...
    def mirostat_update(self, sample_results: SampleResultType):
        ...
        for i, seq_id in enumerate(seq_ids):
            if seq_id.item() in self.tracker._states:
                ...
                # Compute surprise value
                surprise = math.log2(1 / prob)

                # Update running statistics
                state.running_tot_surprise += surprise
                state.error_surprise = surprise - self.mirostat_tau.item()

                # Adjust max_surprise using error
                state.max_surprise -= state.learning_rate * state.error_surprise
        del self.original_logits
    ...
    def _init_sampling_tensors(
        ...
        self.nsigma = self.nsigma.to(logits.device)
    ...
    def forward(
        ...
        if self.sampling_method == "top-nsigma":
            logits = self.nsigma_truncate(logits)
        ...
        if self.sampling_method == "mirostat":
            self.mirostat_update(sample_results)
...
def _beam_search_sample(
    ...
    for seq_group in selected_seq_groups:
        ...
        if is_prompt:
            ...
            seq_group_logprobs = seq_group_logprobs + cumulative_logprobs_tensor.unsqueeze(dim=1)
...
def _sample_with_torch(
    ...
    sample_metadata: Dict[SamplingType, Tuple[List[int], List[SequenceGroupToSample]]] = {}
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            if flashinfer_top_k_top_p_sampling is not None:
                ...
                multinomial_samples[sampling_type] = _multinomial(
                    probs[long_sample_indices], max_best_of_in_batch, seq_groups=seq_groups_arg
                )

            if sampled_token_ids_tensor is not None:
                # Store sampled tokens in output tensor.
                sampled_token_ids_tensor[long_sample_indices] = multinomial_samples[sampling_type].to(
                    torch.long
                )
    ...
    if not sampling_metadata.skip_sampler_cpu_output:
        ...
        sample_results = [
            sample_results_dict.get(i, ([], [])) for i in range(len(sampling_metadata.seq_groups))
        ]
    else:
        sample_results = []
...
def _sample_with_triton_kernel(
    ...
    sample_metadata: Dict[
        SamplingType, Tuple[List[int], List[SequenceGroupToSample], torch.Tensor, torch.Tensor]
    ] = {}
    max_best_of_in_batch = 1
    ...
    for sampling_type in SamplingType:
        ...
        seq_group_id = categorized_seq_group_ids[sampling_type]
        seq_groups = [sampling_metadata.seq_groups[i] for i in seq_group_id]
        sample_metadata[sampling_type] = (seq_group_id, seq_groups, sample_indices, sampled_token_indices)
    ...
    for sampling_type in SamplingType:
        if sampling_type not in sample_metadata:
            continue
        (seq_group_id, seq_groups, sample_indices, sampled_token_indices) = sample_metadata[sampling_type]
        if sampling_type == SamplingType.GREEDY:
            sample_results = _greedy_sample(seq_groups, sampled_tokens[sampled_token_indices][:, 0])
        elif sampling_type in (SamplingType.RANDOM, SamplingType.RANDOM_SEED):
            sample_results = _random_sample(seq_groups, sampled_tokens[sampled_token_indices])
        elif sampling_type == SamplingType.BEAM:
            sample_results = _beam_search_sample(seq_groups, beam_search_logprobs)
        sample_results_dict.update(zip(seq_group_id, sample_results))
...
def _modify_greedy_probs_inplace(
    logprobs: torch.Tensor, probs: torch.Tensor, sample_indices: torch.Tensor, greedy_samples: torch.Tensor
) -> None:
